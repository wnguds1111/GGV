...
# Script to regenerate gnjoy_billing_complete_report.html with a clean, premium LIGHT theme
lines = [
    '<!DOCTYPE html>',
    '<html lang="ko">',
    '<head>',
    ...
    '<title>GNJOY HK - 게임별 결제 채널 완전 분석 보고서 (Light Theme)</title>',
...
lines += [
    ...
    '</div></div>',
    # ROO
    '<div class="oc" style="--gc:#2563EB"><div class="oc-code">ROO</div><div class="oc-nm">愛如初見</div><div class="oc-s">',
    '<div class="oc-r"><span class="oc-rl">결제 탭</span><span class="oc-rv">8개</span></div>',
    ...
    '</div></div>',
    # RORB
    '<div class="oc" style="--gc:#059669"><div class="oc-code">RORB</div><div class="oc-nm">重生</div><div class="oc-s">',
    '<div class="oc-r"><span class="oc-rl">결제 탭</span><span class="oc-rv">8개</span></div>',
    ...
    '</div></div>',
    # ROTL
    '<div class="oc" style="--gc:#D97706"><div class="oc-code">ROTL</div><div class="oc-nm">曙光</div><div class="oc-s">',
...
lines += [
    ...
    '<th style="color:#C8102E">ROBA 彩鑽</th>',
    '<th style="color:#2563EB">ROO 喵喵果實</th>',
    '<th style="color:#059669">RORB 鑽石</th>',
    '<th style="color:#D97706">ROTL 星石</th></tr></thead>',
    '<tbody>',
...
roba_panels = (
    ...
    + cc('#C8102E','🃏','MyCard 點數卡','카드번호+비밀번호 직접입력','면액 (TWD)',['100','200','300','500','1,000','2,000'],'실제 지급 彩鑽 수량은 별도 환율표 적용')
    + cc('#EA580C','🟠','GASH 點數卡 (台灣)','台灣 지역 GASH 포인트카드','면액 (TWD)',['50','100','200','300','500','1,000'],'台灣 지역 선택 후 GASH 카드번호 입력')
    ...
    + cc('#7D5265','💜','港澳信用卡 3D','VISA/MasterCard 港澳 발행','면액 (TWD 환산)',['60','290','1,490','2,990'],'港澳 지역 카드 결제. 환율 자동 적용')
    + '</div></div>'
    ...
    + cc('#0284C7','🍎','Apple Pay','iOS Safari 전용','면액 (TWD)',['60','290','1,490','2,990'],'⚠️ 모바일 Safari 팝업 차단 해제 필요','nw')
    + cc('#16A34A','🤖','街口支付 / Google Pay','Android / 街口 전자지갑','면액 (TWD)',['60','290','1,490','2,990'])
    + cc('#16A34A','💚','LINE Pay','LINE 페이 전자결제','면액 (TWD)',['60','290','1,490','2,990'])
    + '</div></div>'
    + f'<div id="roba-oth" class="pp"><div class="cg">'
    + cc('#2563EB','💙','AlipayHK','香港 알리페이 (港澳 전용)','면액 (HKD)',['30','60','150','300','600'])
    + cc('#059669','💬','WeChat Pay','微信支付 (港澳 전용)','면액 (HKD)',['30','60','150','300'])
    + cc('#DC2626','🏦','PayMe (HSBC)','홍콩 HSBC PayMe','면액 (HKD)',['30','60','150','300'])
    + '</div></div>'
    + f'<div id="roba-ph" class="pp"><div class="cg">'
    + cc('#0284C7','📡','中華電信 QR (QR839)','CHT 통신 소액결제 QR','면액 (TWD)',['30','50','100','150','200','250','300','350','400','450','500'],'⚠️ 3,000 TWD 이상 단건 또는 30일 누적 5,000 이상 시 MID 본인인증 필요','nw')
    + cc('#EA580C','📶','遠傳電信 (FET)','Far EasTone 통신 소액결제','면액 (TWD)',['30','50','100','150','200','250','300','500'])
    + cc('#DC2626','📱','台哥大 myFone (TCC)','Taiwan Mobile 통신 소액결제','면액 (TWD)',['30','50','100','150','200','250','300','500'])
    + '</div></div>'
    + f'<div id="roba-ph2" class="pp"><div class="cg">'
    + cc('#0284C7','☎️','中華電信 HiNet 市話','유선전화 HiNet 요금 합산 결제','면액 (TWD)',['100','200','300','500'],'유선전화 번호 입력 필요. HiNet 가입자 전용')
    + '</div></div>'
    + f'<div id="roba-atm" class="pp"><div class="cg">'
    + cc('#475569','🏧','WebATM 即時轉帳','대만 은행 WebATM 인터넷 뱅킹','면액 (TWD)',['100','300','500','1,000','2,000','3,000','5,000'],'대만 은행 계좌 + WebATM 기기 또는 공인인증 필요')
    + '</div></div>'
)

roba_notice = (
    '<div class="ntc" style="background:#FEF2F2;border:1px solid #FCA5A5;color:#991B1B;">'
    '<h4 style="color:#C8102E">⚠️ ROBA 결제 주의사항</h4><ul>'
...
roo_panels = (
    ...
    + cc('#C8102E','🃏','MyCard 點數卡','카드번호+비밀번호 직접입력','면액 (TWD)',['100','200','300','500','1,000'])
    + cc('#EA580C','🟠','GASH 點數卡 (台灣/港澳)','台灣 · 港澳 지역 선택','면액 (TWD)',['50','100','300','500','1,000'])
    + '</div></div>'
    ...
    + cc('#7D5265','💜','港澳信用卡 3D','VISA/MasterCard 港澳 발행','면액 (HKD 환산 자동)',['30','50','150','300','450','500','750','1,000','2,000','3,000','5,000'])
    + '</div></div>'
    ...
    + cc('#0284C7','🍎','Apple Pay','iOS Safari 전용','면액 (TWD)',['50','150','300','500','1,000'],'⚠️ 모바일 Safari 팝업 차단 해제 필요','nw')
    + cc('#16A34A','🤖','街口支付 / Google Pay','Android 전자지갑','면액 (TWD)',['50','150','300','500'])
    + cc('#16A34A','💚','LINE Pay','LINE 페이 결제','면액 (TWD)',['50','150','300','500'])
    + '</div></div>'
    + f'<div id="roo-oth" class="pp"><div class="cg">'
    + cc('#2563EB','💙','AlipayHK','港澳 전용','면액 (HKD)',['30','60','150','300'])
    + cc('#059669','💬','WeChat Pay','港澳 전용','면액 (HKD)',['30','60','150','300'])
    + cc('#DC2626','🏦','PayMe (HSBC)','홍콩 HSBC','면액 (HKD)',['30','60','150'])
    + '</div></div>'
    + f'<div id="roo-ph" class="pp"><div class="cg">'
    + cc('#0284C7','📡','中華電信 QR (QR839)','CHT 통신 소액결제','면액 (TWD)',['30','50','100','150','200','300','500'],'⚠️ MID 본인인증: 3,000 TWD 이상 단건 또는 30일 누적 5,000 이상','nw')
    + cc('#EA580C','📶','遠傳電信 (FET)','Far EasTone','면액 (TWD)',['30','50','100','200','300','500'])
    + cc('#DC2626','📱','台哥大 myFone (TCC)','Taiwan Mobile','면액 (TWD)',['30','50','100','200','300','500'])
    + '</div></div>'
    + f'<div id="roo-ph2" class="pp"><div class="cg">'
    + cc('#0284C7','☎️','中華電信 HiNet 市話','유선전화 요금 합산','면액 (TWD)',['100','200','300','500'])
    + '</div></div>'
    + f'<div id="roo-atm" class="pp"><div class="cg">'
    + cc('#475569','🏧','WebATM 即時轉帳','대만 은행 인터넷 뱅킹','면액 (TWD)',['100','300','500','1,000','3,000','5,000'])
    + '</div></div>'
)
roo_notice = (
    '<div class="ntc" style="background:#EFF6FF;border:1px solid #BFDBFE;color:#1E40AF;">'
    '<h4 style="color:#2563EB">ℹ️ ROO 결제 특이사항</h4><ul>'
...
rorb_panels = (
    ...
    + cc('#C8102E','🃏','MyCard 點數卡','카드번호+비밀번호 직접입력','면액 (TWD)',['100','200','500','1,000'])
    + cc('#EA580C','🟠','GASH 點數卡 (台灣/港澳)','台灣 · 港澳 지역 선택','면액 (TWD)',['50','100','200','500','1,000'])
    + '</div></div>'
    ...
    + cc('#7D5265','💜','港澳信用卡 3D','VISA/MasterCard 港澳 발행','면액 (TWD 동일)',['50','250','500','1,000','2,500','5,000'])
    + '</div></div>'
    ...
    + cc('#0284C7','🍎','Apple Pay','iOS Safari 전용','면액 (TWD)',['50','250','500','1,000'],'⚠️ 모바일 Safari 팝업 차단 해제 필요','nw')
    + cc('#16A34A','🤖','街口支付 / Google Pay','Android 전자지갑','면액 (TWD)',['50','250','500'])
    + cc('#16A34A','💚','LINE Pay','LINE 페이 결제','면액 (TWD)',['50','250','500'])
    + '</div></div>'
    + f'<div id="rorb-oth" class="pp"><div class="cg">'
    + cc('#2563EB','💙','AlipayHK / WeChat Pay / PayMe','港澳 전용 3개 채널','면액 (HKD)',['30','60','150','300'])
    + '</div></div>'
    + f'<div id="rorb-ph" class="pp"><div class="cg">'
    + cc('#0284C7','📡','中華電信 / 遠傳 / 台哥大','대만 3대 통신사 소액결제','면액 (TWD)',['30','50','100','200','300','500'],'⚠️ 인증된 휴대폰 번호 필요','nw')
    + '</div></div>'
    + f'<div id="rorb-ph2" class="pp"><div class="cg">'
    + cc('#0284C7','☎️','中華電信 HiNet 市話','유선전화 요금 합산','면액 (TWD)',['100','200','300','500'])
    + '</div></div>'
    + f'<div id="rorb-atm" class="pp"><div class="cg">'
    + cc('#475569','🏧','WebATM 即時轉帳','대만 은행 인터넷 뱅킹','면액 (TWD)',['100','300','500','1,000','3,000'])
    + '</div></div>'
)
lines.append(game_section(
...
rotl_notice = (
    ...
    '<h4 style="color:#D97706">⚠️ ROTL 결제 제한 사항</h4><ul>'
...
rotl_panels = (
    ...
    + cc('#C8102E','🃏','MyCard 點數卡','카드번호+비밀번호 직접입력','면액 (TWD)',['100','200','500','1,000'])
    + cc('#EA580C','🟠','GASH 點數卡 (台灣만)','港澳 GASH 카드 미지원','면액 (TWD)',['50','100','200','500'],'⚠️ 台灣 발행 GASH 카드만 지원. 港澳 카드 미지원','nw')
    + '</div></div>'
    ...
    + cc('#0284C7','🍎','Apple Pay','iOS Safari 전용 (제한적)','면액 (TWD)',['60','290','1,490'],'⚠️ 지원 여부 Cloudflare Turnstile 캡차로 확인 불완전','nw')
    + '</div></div>'
)
lines.append(game_section(
...
with open('gnjoy_billing_complete_report.html', 'w', encoding='utf-8') as f:
    f.write(output)
print(f'Done! Light theme written: {len(output)} chars')
